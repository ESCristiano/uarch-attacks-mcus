...
void platform_init(void)
{
    ...
    ///------------------------------------------------------------------------
     clk_rc = Cy_SysClk_PeriphDisableDivider(CY_SYSCLK_DIV_8_BIT, 2U);
    if (clk_rc != CY_SYSCLK_SUCCESS) {
        SPMLOG_INFMSG("WARNING: Failed to configure UART clock\r\n");
    }
    clk_rc = Cy_SysClk_PeriphSetDivider(CY_SYSCLK_DIV_8_BIT, 2U, 1U);
    if (clk_rc != CY_SYSCLK_SUCCESS) {
        SPMLOG_INFMSG("WARNING: Failed to configure UART clock\r\n");
    }
    clk_rc = Cy_SysClk_PeriphEnableDivider(CY_SYSCLK_DIV_8_BIT, 2U);
    if (clk_rc != CY_SYSCLK_SUCCESS) {
        SPMLOG_INFMSG("WARNING: Failed to configure UART clock\r\n");
    }

    /* Secure: TIMER0 clock */
    clk_rc = Cy_SysClk_PeriphAssignDivider(PCLK_TCPWM0_CLOCKS0, CY_SYSCLK_DIV_8_BIT, 2U);
    if (clk_rc != CY_SYSCLK_SUCCESS) {
